...
% x=read_bin('resultSW11_T1451009422_NQ11976.bin',floor(6592*1.8), 11919*2);
%                                      ^^^^^                      ^ NQx2 ^
...
if (f<0)
    ...
    IW=str2double(name(9:10));  % swath number
    if (IW==10)
...
% range compression 
  ...
  phi2=TXPRR/2;
  nomchirp=exp(1i*2*pi*(phi1*tim+phi2*tim.^2));
  ...
  for k=1:count % size(t)(1)   % 6592 = sweep along azimuth for range compression
    ...
    t=t(1:2:end)+t(2:2:end)*1i; % see GNU Radio's read_complex_binary
    disp(k);
...
figure
imagesc(rot90(abs(ssol(1:3:end,1:3:end)),2),[1 2e8]);
